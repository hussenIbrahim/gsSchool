...
class BehaveNotifer extends ChangeNotifier {
  Status getDataStatus = Status.initState;
  List<BehaveModel> eventsList = [];
  final services = locator<BehaveServices>();
  ...
  setFromHive(List<BehaveModel> list) {
    eventsList = list;
  ...
  getPicksForYouOffers() async {
    BehaveModelResponse data = await services.getChatPersons(page);
    getDataStatus = data.status;
    myPrint("BehaveModelResponse data  $data");

    if (getDataStatus == Status.dataFound) {
      page = page + 1;
      final List<BehaveModel> _productModelList = data.data;
